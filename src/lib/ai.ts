...
export async function generateChecklist(task: {
  ...
  const systemPrompt = `あなたは業務タスク管理アシスタントです。
タスクのタイトルと概要をもとに、具体的な作業チェックリスト項目を日本語で5〜7件生成してください。

出力形式: JSON配列のみ（他のテキスト一切なし）
例: ["項目1", "項目2", "項目3"]
...
- 1項目は25文字以内
- 業務の流れに沿った順番で並べる
- 汎用的すぎる項目（「確認する」だけなど）は避ける`;

  const userMessage = `タスク名: ${task.title}\n概要: ${task.description || '（未設定）'}`;

  const raw = await callAIForJson(systemPrompt, userMessage);
  const jsonMatch = raw.match(/\[[\s\S]*?\]/);
  if (!jsonMatch) throw new Error('チェックリストの生成に失敗しました');
...
export async function suggestAutoCheck(task: {
  title: string;
  checklistItems: string[];
  memoContent: string;
}): Promise<string[]> {
  const systemPrompt = `あなたは業務タスク管理アシスタントです。
今日のメモ内容とタスクのチェックリスト項目を照合し、「完了済みと判断できる」項目のテキストをJSON配列で返してください。
...
ルール:
- メモに完了・対応済みを示す記述がある場合のみ含める
- 推測・憶測は含めない
- チェックリスト項目のテキストを一字一句そのまま返す`;

  const userMessage = `【タスク名】${task.title}

【未完了チェックリスト項目】
${task.checklistItems.map((item, i) => `${i + 1}. ${item}`).join('\n')}
...
export async function generateTaskProgressComment(task: {
  title: string;
  description: string;
  progress: number;
  ...
  const systemPrompt = `あなたは執事型AIアシスタント「Sebastian」です。
タスクの現在の進捗状況をもとに、ご主人様への一言コメントを生成してください。

ルール:
- 50〜90文字程度の日本語
- 執事らしい丁寧・控えめな口調
- 進捗に応じた励まし、または次のアクションの示唆を含める
- 「ございます」「でございます」「お疲れ様でございました」などの敬語を使う`;

  const checkInfo = task.checklistTotal > 0
    ? `チェックリスト: ${task.checklistDone}/${task.checklistTotal}件完了`
    : 'チェックリストなし';
