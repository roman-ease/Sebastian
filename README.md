...
- カテゴリフィルター（登録済みカテゴリをドロップダウンで絞り込み。リスト上のカテゴリ名クリックでもワンタップ絞り込み可）
- ソートキー選択（作成日 / 更新日 / 期日 / 優先度）
- 昇降順切り替えボタン
...
- AI無効時は定型の執事フレーズをフォールバック表示

#### 週スケジュール — サマリーパネル追加
- 各曜日カラム下端に**タスク負荷バー**を追加（タスク数に応じて伸縮・変色）
- カレンダー下部に切り替え可能なパネルを追加
  - **週のサマリー**: 期日タスク合計・完了数・未完了数・高優先度数・達成率バー・週評AIコメント
  - **今週の格言**: 著名人（哲学者・作家・歴史上の人物）の名言を週番号でローテーション、セバスチャンが口上付きで紹介

#### 日報生成 — 品質改善
- Gemini API の `maxOutputTokens` を 2048 → 8192 に引き上げ（長文メモでの途中打ち切りを解消）
- `finishReason: MAX_TOKENS` 検知を追加し、打ち切り時に明示的なエラーメッセージを表示
